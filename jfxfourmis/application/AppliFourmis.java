...
public class AppliFourmis extends Application {
	...
	void construireScenePourFourmis(Stage primaryStage) 
	{
		...
		AppliFourmis.environnement = new Rectangle[taille][taille];
	...
	void dessinEnvironnement(Group root)
	{
		...
		for(int i=0; i<taille; i++)
			...
			{
				AppliFourmis.environnement[i][j] = new Rectangle((i+1)*(espace), (j+1)*(espace), espace, espace);
				AppliFourmis.environnement[i][j].setFill(Color.DARKGREEN);
				root.getChildren().add(AppliFourmis.environnement[i][j]);
			}
		for(int i=0; i<taille; i++)
			for(int j=0; j<taille; j++)
			{
				...
				if (cell.getNourriture()>0) // affichage des zones de nourritures 
				{
					...
					AppliFourmis.environnement[i][j] = new Rectangle((i+1)*(espace), (j+1)*(espace), espace, espace);
					AppliFourmis.environnement[i][j].setFill(colNouriture);
					...
					{
						AppliFourmis.environnement[i][j] = new Rectangle((i+1)*(espace), (j+1)*(espace), espace, espace);
						AppliFourmis.environnement[i][j].setFill(Color.BROWN);
						// dessin des nids � la place de l'environnement
						root.getChildren().remove(AppliFourmis.environnement[i][j] );
						root.getChildren().add(AppliFourmis.environnement[i][j]);
					}
			}
		...
		for(int i=0; i<taille; i++)
			for(int j=0; j<taille; j++)
			{
				...
				if (cell.hasJustChanged && cell.getPheromone()>0 && cell.getNourriture()==0  && !cell.isNid() ) // s'il y a une trace de ph�romone
				{
					...
					AppliFourmis.environnement[i][j].setFill(c);
				}
				if (cell.hasJustChanged && cell.getPheromone()==0 && cell.getNourriture()==0 && !cell.isNid()) // s'il y a une trace de ph�romone
				{
					AppliFourmis.environnement[i][j].setFill(Color.DARKGREEN);
